...
try {
    $stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'kid'");
    $stmt->execute();
...
<body>
    ...
    <section class="main-gallery">
        <?php if (empty($products)): ?>
            <p style="text-align:center;">No kid's products available.</p>
        <?php else: ?>
            <?php 
            $first_seven = array_slice($products, 0, 7);
            $first_four = array_slice($first_seven, 0, 4);
            $next_three = array_slice($first_seven, 4, 3);
            ?>
            
            <!-- First row of 4 products -->
            <div class="gallery-row-4">
            <?php foreach ($first_four as $product): ?>
            ...
            <?php foreach ($next_three as $product): ?>
                <div class="gallery-item" onclick="showProductModal(this)" data-id="<?= $product['id']; ?>"
                    data-name="<?= htmlspecialchars($product['name']); ?>" data-price="<?= $product['price']; ?>"
                    data-image="<?= htmlspecialchars($product['image']); ?>">
                    <img src="<?= htmlspecialchars($product['image']); ?>" alt="<?= htmlspecialchars($product['name']); ?>">
    ...
    <section class="main-gallery">
        <?php if (!empty($products)): ?>
            <?php 
            $last_seven = array_slice($products, -7);
            $last_four = array_slice($last_seven, 0, 4);
            $final_three = array_slice($last_seven, 4, 3);
            ?>
            
            <!-- First row of 4 products -->
            <div class="gallery-row-4">
            <?php foreach ($last_four as $product): ?>
                <div class="gallery-item" onclick="showProductModal(this)" data-id="<?= $product['id']; ?>"
                    data-name="<?= htmlspecialchars($product['name']); ?>" data-price="<?= $product['price']; ?>"
                    data-image="<?= htmlspecialchars($product['image']); ?>">
                    <img src="<?= htmlspecialchars($product['image']); ?>" alt="<?= htmlspecialchars($product['name']); ?>">
            ...
            <!-- Second row of 3 products -->
            <div class="gallery-row-3">
            <?php foreach ($final_three as $product): ?>
                <div class="gallery-item" onclick="showProductModal(this)" data-id="<?= $product['id']; ?>"
                    data-name="<?= htmlspecialchars($product['name']); ?>" data-price="<?= $product['price']; ?>"
                    data-image="<?= htmlspecialchars($product['image']); ?>">
                    <img src="<?= htmlspecialchars($product['image']); ?>" alt="<?= htmlspecialchars($product['name']); ?>">
